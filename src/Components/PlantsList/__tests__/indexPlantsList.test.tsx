...
const mockGetPlantsListForUser = jest.fn(() => Promise.resolve(console.log('Downloading plants lists...')));
...
describe('PlantsList component', () => {
  it('Should render without error', () => {
    ...
    expect(stackScreens.at(0).prop('name')).toBe(`${initialState.plantsLists[0].id}`);
    expect(stackScreens.at(1).prop('name')).toBe(`${initialState.plantsLists[1].id}`);
    expect(stackScreens.at(2).prop('name')).toBe(`${initialState.plantsLists[2].id}`);
  });
});
